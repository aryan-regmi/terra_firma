...
impl PlayerPlugin {
    ...
    fn setup(mut cmd: Commands, asset_server: Res<AssetServer>) {
        // Spawn character
        let player = asset_server.load("tileset/character-00.png");
        cmd.spawn((
            Player,
            Sprite::from_image(player),
            Transform::from_xyz(0., 0., Self::PLAYER_Z_IDX).with_scale(Vec3::splat(1.2)),
        ));
    }
    ...
    fn update(
        ...
        let updated_transform_x =
            player_position.translation.x + x_direction * Self::PLAYER_SPEED * time.delta_secs();
        let updated_transform_y =
            player_position.translation.y + y_direction * Self::PLAYER_SPEED * time.delta_secs();
